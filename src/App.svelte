<script>
	...
	$: username = gameRoom.host_username;
	...
	async function* gameStep() {
		...
		maxSpells = Dice.roll();
		gameRoom.stats.spells = maxSpells;
		yield;
		...
		while(player1.hp > 0 && player2.hp > 0) {
			...
			// gameRoom = Object.assign({}, gameRoom);
			// Player decides
			if (Dice.roll() % 2 === 0) {
		...
		<div class="bg-red-200 p-2 grid gap-2">
			{#each player1.selectedSpells as spell, i}
				<button class="bg-white rounded-4 border border-gray-500 active:bg-gray-100 text-12 p-2 w-full">
					{i+1}. {spell.name} ({spell.level}) {username}
				</button>
			{/each}
		</div>
